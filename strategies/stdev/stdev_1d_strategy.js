...
const PlaygroundStrategy = (isBacktest, std_multiplier) => {
  ...
  const isBuyingTime = async ({
    ...
    try {
      ...
      const median = math.median(returns);
      /* if (
        returns[returns.length - 1] < mean - std * std_multiplier &&
        returns[returns.length - 1] < 0
      ) {
        return true;
      } */
      if (
        returns[returns.length - 1] < median - std * std_multiplier &&
        returns[returns.length - 1] < 0
      ) {
        return true;
      }
