...
const generateData = async (symbol, begin, interval) => {
  try {
    let oldData = { id: "" };
    const newId = `${symbol}_${interval}`;
    try {
    ...
    let newData = [];
    try {
      newData = JSON.parse(fs.readFileSync(generateFileName(`${symbol}_1d`)))
        .data;
    } catch (_) {
      newData = await getKlines(symbol, begin);
      fs.writeFileSync(
        generateFileName(`${symbol}_1d`),
        JSON.stringify({
          id: `${symbol}_1d`,
          data: newData,
        })
